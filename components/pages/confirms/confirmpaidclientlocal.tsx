...
export default function ConfirmPaymentClient() {
  ...
  useEffect(() => {
    ...
    const processTransaction = async () => {
      try {
        // 1️⃣ Confirmamos pago con Payphone
        const result = await confirmPayment(id, clientTransactionId);

        if (!result.success || !result.data) {
          setError(result.error || "Error desconocido al confirmar pago");
          return;
        }

        // 🔹 Recuperamos invoiceNumber
        const invoiceNumber = localStorage.getItem("invoiceNumber") || "";

        // 2️⃣ Enviamos al backend
        const backendResponse = await fetch("/api/backend/payments/confirm", {
          method: "POST",
          headers: { "Content-Type": "application/json" },
          body: JSON.stringify({
            transactionId: result.data.transactionId,
            clientTransactionId: result.data.clientTransactionId,
            invoiceNumber,
          }),
        });
        ...
        if (!backendResult.success) {
          setError(
            backendResult.error || "Error desconocido al registrar en backend",
          );
          return;
        }

        // 🔹 Confirmado backend → eliminamos localStorage y seteamos estado
        localStorage.removeItem("invoiceNumber");
        setTransactionData(result.data);

        // 🔹 Solo después de backend, cerramos ventana si aprobado
        if (result.data.transactionStatus === "Approved") {
